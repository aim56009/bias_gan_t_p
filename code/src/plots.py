...
from bias_gan_t_p.code.src.data import TestData
...
class PlotAnalysis():
    ...
    def single_frames_pr(self, vmin=0,
                              vmax=20,
                              time_index=-1,
                              cmap='Blues',
                              mask=False,
        ...
        for ax, (name, letter) in zip(axs, zip(name, letters)):
            data_pr = abs(getattr(self.data, name).precipitation.isel(time=time_index))
            ...
            plt.title("test data " + self.data.model_name_definition(name) + " precipitation")

            cbar = False
            cbar_title = ''

            cs = plot_basemap(data_pr, cbar_title, vmin, vmax, alpha, cmap,
                        cbar=cbar,
                        axis=ax,
                        return_cs=True,
    ...
    def single_frames_t(self, vmin=0,
                              vmax=20,
                              time_index=-1,
                              cmap='Blues',
                              mask=False,
        ...
        for ax, (name, letter) in zip(axs, zip(name, letters)):
            data_t = abs(getattr(self.data, name).tas.isel(time=time_index))
            if name=="era5" :
            ...
            ax.annotate(f"{letter}", ha="center", va="center", size=15,
                        ...
                        bbox=None) 
            ax.set_title(self.data.model_name_definition(name))
            plt.title("test data " + self.data.model_name_definition(name) + " Temperature")

            cbar = False
            cbar_title = ''

            cs = plot_basemap(data_t, cbar_title, vmin, vmax, alpha, cmap,
                        cbar=cbar,
                        axis=ax,
                        return_cs=True,
                        projection=projection,
    ...
    def histograms_pr(self, single_plot=False, ax=None, show_legend=True, annotate=True,log=True,xlim_end=400):
        ...
        for name in reversed(self.names):
            ...
            data = getattr(self.data, name).precipitation.values.flatten()
            era5_data = getattr(self.data, "era5").precipitation.values.flatten()
            if name != "era5":
    ...
    def histograms_t(self, single_plot=False, ax=None, show_legend=True, annotate=True,log=True,xlim_end=400):
        ...
        for name in reversed(self.names):
            ...
            data = getattr(self.data, name).tas.values.flatten()
            era5_data = getattr(self.data, "era5").tas.values.flatten()
            if name != "era5":
              print(f" Absolute Difference era5&{name} (in test period):",int(np.sum(era5_data)-np.sum(data)))
              print('')
              print(f" Absolute Difference in percent era5&{name}",abs(np.round((np.sum(era5_data)-np.sum(data))/np.sum(era5_data),5)))
    ...
    def latitudinal_mean_pr(self, single_plot=False, ax=None, show_legend=True, annotate=True):
        if single_plot:
        ...
        for name in reversed(names):
            ...
            data = getattr(self.data, name).precipitation
            label = self.data.model_name_definition(name)
            if name != "era5":
                era5_data = getattr(self.data, "era5").precipitation
    ...
    def latitudinal_mean_t(self, single_plot=False, ax=None, show_legend=True, annotate=True):
        if single_plot:
            plt.figure(figsize=(12,8))
        names = self.names
        for name in reversed(names):
            ...
            data = getattr(self.data, name).tas
            if name=="gan" :
                data = data.rename({'latitude': 'lat', 'longitude': 'lon'})

            label = self.data.model_name_definition(name)
            if name != "era5":
                era5_data = getattr(self.data, "era5").tas
                bias = data.mean('time') - era5_data.mean('time') 
                print(label,f" \t \t MAE: {abs(bias).values.mean():2.3f} [mm/d]")
    ...
    def histogram_diff_pr(self, single_plot=False, ax=None, show_legend=True, annotate=True,log=True,xlim_end=500):
    ...
    def histogram_diff_t(self, single_plot=False, ax=None, show_legend=True, annotate=True,log=True,xlim_end=500):
